# Hi, I'm Jekaterina 👋

I'm a career changer from luxury hospitality who fell in love with building things on the web. After years of creating seamless guest experiences, I now focus on creating seamless digital ones.

Based in Barcelona 🌍 | Open to junior developer roles 💼

🔗 [My Portfolio](https://jekaterinaa.surge.sh/)

---

## About Me

My background in hospitality taught me that the best experiences are invisible - everything just *works*, and people feel good. That same philosophy drives how I approach development: clean structure, thoughtful UX, and systems that make life easier.

I bring strong soft skills that most junior devs are still developing - working under pressure, attention to detail, cross-cultural communication, and a genuine obsession with getting things right.

I'm now channeling all of that into code.

---

## What I'm Building

### 🛍️ Ease — Mood-Based Store
A web app that personalises the shopping experience based on how you're feeling. Because what you need on a hard day is different from what you want on a great one.

**Tech:** HTML/CSS · JavaScript · React.js · Node.js · Express.js · MongoDB

🔗 [Live Demo](https://mood-based-store-project.vercel.app/)

---

### ⚡ Boost&Burn — Social Energy Simulator
A mobile app that helps you track your daily energy levels, log activities, and understand what genuinely boosts or drains you throughout the day.

**Tech:** React Native · Expo

🔗 [App Store](https://apps.apple.com/es/app/boost-burn/id6760339479?l=en-GB)
🔗 [Google Play](https://play.google.com/store/apps/details?id=com.barcelonacodeschool.boostburn)

---

## Tech Stack

![HTML](https://img.shields.io/badge/HTML5-E34F26?style=flat&logo=html5&logoColor=white)
![CSS](https://img.shields.io/badge/CSS3-1572B6?style=flat&logo=css3&logoColor=white)
![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?style=flat&logo=javascript&logoColor=black)
![TypeScript](https://img.shields.io/badge/TypeScript-3178C6?style=flat&logo=typescript&logoColor=white)
![React](https://img.shields.io/badge/React-20232A?style=flat&logo=react&logoColor=61DAFB)
![Node.js](https://img.shields.io/badge/Node.js-339933?style=flat&logo=nodedotjs&logoColor=white)
![Express](https://img.shields.io/badge/Express.js-000000?style=flat&logo=express&logoColor=white)
![React Native](https://img.shields.io/badge/React_Native-20232A?style=flat&logo=react&logoColor=61DAFB)
![Expo](https://img.shields.io/badge/Expo-000020?style=flat&logo=expo&logoColor=white)
![MongoDB](https://img.shields.io/badge/MongoDB-4EA94B?style=flat&logo=mongodb&logoColor=white)
![Git](https://img.shields.io/badge/Git-F05032?style=flat&logo=git&logoColor=white)

---

## Education

**Full-Stack Web & Mobile Development Bootcamp** · Barcelona Code School *(January-March 2026)*  
An intensive in-person programme covering the full stack - from frontend fundamentals to mobile development with React Native.

---

## What Drives Me

- Building things that feel intuitive and human
- Learning by doing - and then doing it better
- Combining structure with creativity
- Systems thinking: understanding the *why* behind the *how*

---

## Let's Connect

I'm actively looking for junior developer opportunities where I can keep growing, contribute meaningfully, and be part of a team that cares about craft.

- [LinkedIn](https://www.linkedin.com/in/jekaterina-antoine-01634b106/)
- [Email](mailto:jekaterinaantoine@gmail.com)
  
---

Thank you for visiting my profile ✨
